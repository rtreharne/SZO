...
class Page:
    ...
    def crunch(self, p):
        ...
        print 'mu0=', mu0
        mu1 = mu0*exp(-Eb/kT)
        mu2 = mu1*(exp(-(A/e)*(Eb-Ef)))
        mu_tot = ((1/mu2)+(1/mumax))**(-1)
        return mu2

    def update(self, x, mu1):
        sigma = 1e-20
        ...
        #leg1 = self.ax.legend((data1, line1, line2,line3), ('data ($\leq 0.65 \%$ wt. SiO$_{2}$)',r'$\mu_{gb}=\mu_0\exp(-\frac{\phi}{k_BT})$', r'$\mu_t=\frac{\mu_{ii}-\mu_{gb}}{1+\exp[-\frac{1}{\alpha}(E_{BM} +E_R-\beta\phi)]}$',r'$\mu_{eff}=\mu_{gb} + \mu_t$'), 'upper left', prop = fontP,fancybox=False)
        #sleg1.get_frame().set_alpha(0.0)
    ...
    def func(self, p):
        ...
        for i in range (0, len(x)):
            ...
            mu2.append(mu1[i]*(exp(-(A/e)*(Eb[i]-Ef[i]))))
