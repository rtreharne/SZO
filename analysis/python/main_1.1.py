...
class Plot():
    ...
    def xy(self, widget):
        ...
        self.ax1.plot(x, y1*1e-26, 'o', color = 'black', alpha = 0.5, markersize = 3)
        self.ax1.set_xlim(0, 6)
        self.ax1.set_ylim(0, 5.5)
        self.ax1.set_yticks([0,1,2,3,4,5])
        self.ax1.xaxis.tick_top()
        self.ax1.set_ylabel(r'Carrier Conc., $n_{e}$ ($\times 10^{20}$ cm$^{-3}$)', fontsize = 11)
  

        self.ax2.plot(x, y2, 'o', color = 'black', alpha = 0.5, markersize = 3)
        self.ax2.set_xlim(0, 6)
        self.ax2.set_ylim(0,18)
        self.ax2.set_yticks([0,5,10,15])
        setp(self.ax2.get_xticklabels(), visible=False)
        self.ax2.set_ylabel('Mobility, $\mu_{e}$ (cm$^2$V$^{-1}$s$^{-1}$)')

        self.ax3.plot(x, y3, 'o', color = 'black', alpha = 0.5, markersize = 3)
        self.ax3.set_xlim(0, 6)
        #setp(self.ax3.get_yticklabels(), rotation=45)
        self.ax3.set_ylim(1e-4, 0.5)
        self.ax3.set_yscale('log')
        #self.ax3.set_ylim(0,17)y
        self.ax3.set_xlabel('% wt. SiO$_{2}$ content', fontsize = 14)
        self.ax3.set_ylabel(r'Resistivity, $\rho$ ($\Omega$.cm)')
        u = np.linspace(0, 2, 100)
        v = (u/100)*6.02e29*5.61*2*1e-26/60.08
        w = v/2

        self.ax1.plot(u, v, '-', linewidth=1, color = 'gray')
